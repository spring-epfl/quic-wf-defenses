...
def extract_features(data, max_size=123):
    ...
    incoming = [pkt for pkt in data if pkt[1] > 0]
    outgoing = [pkt for pkt in data if pkt[2] > 0]
    ...
    features['bytes_incoming'] = sum([d[1] for d in incoming])
...
def addPackets(minTime, maxTime, totalSize, maxPacketSize, direction):
    ...
    while curSize < totalSize:
        ...
        curSize += maxPacketSize
        newPackets.append([0, t, maxPacketSize])

    newPackets.sort(key=lambda row: row[1])

    newPackets[-1][2] -= (curSize - totalSize) # last packet has the exact size
...
def defend_nototalsize(trace):
    ...
    # pad total size
    sizeOut = sum([p[2] for p in trace if p[2]>0])
    sizeInc = sum([p[2] for p in trace if p[2]<0])
    ...
    if sizeOut % padsize != 0:
        newPackets = addPackets(trace[-1][1], trace[-1][1]+10, abs(padsize - (sizeOut%padsize)), 1000, 1) # between [end, end+10s]
        trace.extend(newPackets)

    if sizeInc % padsize != 0:
        newPackets = addPackets(trace[-1][1], trace[-1][1]+10, abs(padsize - (sizeInc%padsize)), 1000, -1) # between [end, end+10s]
        trace.extend(newPackets)

    return defend(trace, discard_sizes=True)


def defend_nosize(trace):
    return defend(trace, discard_sizes=True)

def defend_notimes(trace):
    return defend(trace, discard_timings=True)

def defend_dummies(trace):

    trace2 = [[x[0], x[1], x[2]] for x in trace]
    if len(trace2) == 0:
        trace2 = []

    max_out = 500
    max_in = 1000 # 1 kB
    end_t = 3 # 3 sec, arbitrary

    if len(trace2) > 0:
        max_out = np.mean([x[1] for x in trace2])
        max_in = np.mean([x[2] for x in trace2])
        end_t = trace2[-1][0]

    added_rx = 0
    added_tx = 0
    
    n_additional = int(np.random.uniform(0, 50))
    for i in range(n_additional):
        t = np.random.uniform(0, end_t)
        request = np.random.uniform(0, max_out)
        response = np.random.uniform(0, max_in)
        trace2.append([t, request, response])

        added_tx += request
        added_rx += response

    added_tx = round(added_tx / 1024)
    added_rx = round(added_rx / 1024)

    print(f"Added {n_additional} requests, total added {added_tx} tx {added_rx} rx [kB].")
    
    trace2.sort(key=lambda p: p[0])
    return trace2
...
def sample_not_below(thres=0, fn=None):
    res = fn()
    if res < thres:
        shift_desired = thres-fn()
        res = shift_desired + fn()
    return res

def percentage(a, tot):
    if tot == 0:
        return 0
    return round(float(a)/(a+tot))

def defend_smart_dummies(trace, load=1):
    global defense_costs
    ...
    target_n_resources = n_resources + np.random.poisson(lam=int(28 * load)) #sample_not_below(thres=n_resources, fn=lambda: np.random.poisson(lam=int(28 * load)))
    resources_to_add = target_n_resources - n_resources
    
    queries = [request for request in trace if request[1] > 0] # upstream size is > 0

    b_out = sum([subresource[1] for subresource in trace])
    b_in = sum([subresource[2] for subresource in trace])

    root = [0,0,0] # fake subrequests are attached to one "root", any outgoing request (as if it was requested along)
    if len(queries) > 1:
        queries = queries[1:] # can't be attached to the very initial query, that's always just the HTML
        root = queries[0]
    ...
    while resources_to_add > 0:
        ...
        t = root[0]+abs(distrib_small_time_after())

        new_query = [t, distrib_upstream_size(), distrib_downstream_size()]
        #print("Adding fake query", new_query)
        
        extra_cost_messages += 1
        extra_cost_kB += new_query[1] + new_query[2]

        trace.append(new_query)
        resources_to_add -= 1

    trace.sort(key=lambda p: p[0])

    defense_costs.append([extra_cost_messages, extra_cost_kB])

    b_out2 = sum([subresource[1] for subresource in trace])
    b_in2 = sum([subresource[2] for subresource in trace])

    perc1 = 100.0 * percentage(b_in, b_out+b_in)
    perc2 = 100.0 * percentage(b_in2, b_out2+b_in2)

    print(f"Added {target_n_resources - n_resources}, Percentages {perc1} -> {perc2}")

    print(f"Extra cost: {extra_cost_messages} msgs {round(extra_cost_kB/1024)} kB")
...
def build(har_file, output_name=None, defense=None, VARIANT='nofilter'):
    global defense_costs
    defense_costs = []

    result = dict(feature_names=None, feature=[], label=[])
    raw_data = np.load(har_file, allow_pickle=True).item()
    ...
    for i, url in enumerate(raw_data[VARIANT]):
        print(f"[{i}] Processing {url}...", end="\r")

        samples = raw_data[VARIANT][url]

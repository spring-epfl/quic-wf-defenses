...
def rf_folds(train_data, train_labels, test_data, test_labels, feature_names, rfe_nfeatures=50, rfe_steps=100, n_trees=30, n_classes=-1):
    ...
    #    in zip(sss.split(train_data, train_labels), sss.split(test_data, test_labels)):
...
def exclude(features, labels, failed):
...
def process_npy(dataset_npy):

    failed = [166, 217, 201, 169, 210, 84, 104, 234, 87, 241]

    print("Processing {}".format(dataset_npy))

    dic = np.load(dataset_npy, allow_pickle=True).item()
    feature_names = dic["feature_names"]

    features, labels = exclude(dic["features"], dic["labels"], failed)
    ...
    return X, y_i, feature_names

def run(permutation):

    print("permutation:", permutation)

    train_data, train_labels, feature_names = process_npy(permutation[0])
    test_data, test_labels, feature_names = process_npy(permutation[1])
    ...
    if (train_data != 'N/A') & (test_data != 'N/A'):
        ...
        print(score)
        tag = permutation[0].replace('datasets/', '')
        print(tag)
        #plot_builder.confusion_matrix(permutation[0].replace('datasets/', '') + "-cm", y_test_all, y_pred_all)
        #plot_builder.feature_importance(permutation[0].replace('datasets/', '') +"-fi", features_and_percentages)

...
ACCEPTABLE_RESPONSES = set(
    [101, 200, 201, 203, 202, 206, 301, 308, 302, 303, 307, 304, 423])
...
def parse_har(file):
    ...
    for request in requests:
        ...
        query_size = (request['request']['headersSize'],
                      request['request']['bodySize'])
        answer_size = (request['response']['headersSize'],
                       request['response']['bodySize'])

        request_headers = dict_from_headers(request['request']['headers'])
        response_headers = dict_from_headers(request['response']['headers'])
        ...
        if root_host is None and ('host' not in request_headers or request_headers['host'] != expected_host):
            # print("Probable overlapping requests, skipping request {} in file {}.".format(
            #    url, file))
            continue

        # responses are given by the Edge which have H3 enabled
        if int(request['response']['status']) not in ACCEPTABLE_RESPONSES:
            # print("Skipping {} response {} in file {}.".format(request['response']['status'],
            #                                                  url, file))
            continue
        ...
        uses_quic = False
        if 'alt-svc' in response_headers and H3_PATTERN.match(response_headers['alt-svc']):
            uses_quic = True

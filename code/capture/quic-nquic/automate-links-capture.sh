...
# Careful, merging ${OUTPUT_DIR} could be good or bad, better double check
if [ -d ${OUTPUT_DIR} ]; then
    while true; do
        read -p "Careful: Output folder \"${OUTPUT_DIR}\" already exists. Contents will be merged, pre-existing data will be skipped. Impartial captures will be overwritten. Continue ? [yn] " yn
        case $yn in
            [Yy]* ) break;;
            [Nn]* ) exit;;
            * ) echo "Please answer [y]es or [n]o.";;
...
mkdir -p "${OUTPUT_DIR}"
chmod ugo+rwx "${OUTPUT_DIR}" #tshark sometimes chokes on this

for urls in "${INPUT_DIR}/*"
do
    ...
    do
        ...
        do
            ...
            pcap="${OUTPUT_DIR}/${urls}/${filename}/${i}/${PROFILE}/capture.pcap"

            if [ -f "${pcap}" ] && [ -s "${pcap}" ]; then
                t=$(date +%T)
                echo "[${realname}, ${count}, ${t}, ${mod}, ${inc}] Crawling ${url}, ${i}, skipped, pcap exists."
            else
                rm -rf "${OUTPUT_DIR}/${urls}/${filename}/${i}/"
                mkdir -p "${OUTPUT_DIR}/${urls}/${filename}/${i}/"

                t=$(date +%T)
                echo "[${realname}, ${count}, ${t}, ${mod}, ${inc}] Crawling ${url}, ${i}, output in ${OUTPUT_DIR}/${urls}/${filename}/${i}."

                # crawl; wait 70 sec, afterwards send HUP, and send KILL after 10 additional seconds still if not done
                timeout -s HUP -k 80 70 ./capture-url.sh "${CAPTURE_INTERFACE}" "${url}" "${PROFILE}" "${OUTPUT_DIR}/${urls}/${filename}/${i}" > "${OUTPUT_DIR}/${urls}/${filename}/${i}/automation_log.txt" 2>&1
                ...
                if [ "${exitcode}" == 124 ]; then
                    ...
                    echo "[${realname}, ${count}, ${t}, ${mod}, ${inc}] Website behind CAPTCHA, skipping website..." | tee "${OUTPUT_DIR}/${urls}/${filename}/warning"
                    break
                elif [ "${exitcode}" == 3 ]; then
                    echo "[${realname}, ${count}, ${t}, ${mod}, ${inc}] We are being rate-limited by Cloudflare, skipping website..." | tee "${OUTPUT_DIR}/${urls}/${filename}/warning"
                    break
                elif [ "${exitcode}" == 4 ] ||  [ "${exitcode}" == 5 ]; then
                    echo "[${realname}, ${count}, ${t}, ${mod}, ${inc}] Website throwed an exception (perhaps 404?), skipping website..." | tee "${OUTPUT_DIR}/${urls}/${filename}/warning"
                    break

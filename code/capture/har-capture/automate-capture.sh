...
memkill() {
    pkill -f geckodriver
    pkill -f browsermob-proxy
    pkill -f browsermob-prox
    pkill -f ./bin/browsermob-prox
    pkill -f java
    pkill -f firefox
}
...
do
    ...
    if [ -f "${har}" ] && [ -s "${har}" ]; then
        ...
        timeout -s HUP -k 10 60 python3 -u firefox-har-query.py "${url}" "${OUTPUT_FOLDER}"

        memkill
        sleep 10 # important! otherwise things might start to overlap in the proxy
    fi
done < urls
